...
export class DashboardComponent implements OnInit {
    ...
    constructor(
        ...
        this.authService.getExpense().subscribe(abc => {
            ...
            for (let i = 0; i < this.expenses.length; i++) {
                temp.push(this.expenses[i]["date"]);
            }
            ...
            for(let i=0; i<abcd.length; i++){
                if(i==0 && abcd.length>0){
                    ...
                    if(new Date(Date.parse(abcd[i]["date"])).getMonth()==new Date(Date.parse(abcd[i-1]["date"])).getMonth()){
    ...
    onsubmit2(){
        let now = new Date();
        this.dt.setHours(this.mytime.getHours() + 5);
        this.dt.setMinutes(this.mytime.getMinutes() + 30);
